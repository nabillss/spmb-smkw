...
<body>
        ...
        <div class="content-body">
            ...
            <form id="editForm" action="{{ url('/edit_pendaftar/'.$pendaftar->id_pendaftar) }}" method="POST">
                    ...
                    <div class="row-2">
                        <div class="field">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                <input type="text" name="tempat_lahir" class="req" placeholder="Tempat Lahir" value="{{ old('tempat_lahir', $pendaftar->tempat_lahir) }}">
                                <input type="date" name="tanggal_lahir" class="req" value="{{ old('tanggal_lahir', optional($pendaftar->tanggal_lahir)->format('Y-m-d') ?? $pendaftar->tanggal_lahir) }}">
                            </div>
                        </div>
                        <div class="field">
                ...
                <div class="section">
                    ...
                    <div class="row-2">
                        ...
                        <div class="field"><label>Kabupaten / Kota</label>
                            <select name="kabupaten" id="select_kabupaten" disabled>
                    ...
                    <div class="row-2">
                        <div class="field"><label>Kecamatan</label>
                            <select name="kecamatan" id="select_kecamatan" disabled>
                                <option value="">Pilih Kecamatan</option>
                            </select>
                        </div>
                        <div class="field"><label>Desa / Kelurahan</label>
                            <select name="desa" id="select_kelurahan" disabled>
                ...
                <div class="section">
                    ...
                    <div class="row-2">
                        <div class="field">
                            ...
                            <select name="gelombang" class="req" id="select_gelombang" onchange="toggleMetodePembayaranField(this.value)">
                        ...
                        {{-- Catatan: kolom metode_pembayaran belum divalidasi di AdminController@updatePendaftar,
                             jadi field ini murni tampilan dan belum ikut kesimpen kalau diubah.
                             Tambahkan 'metode_pembayaran' => 'nullable|string' di $validated controller kalau mau ini aktif. --}}
                        @php $metodePembayaran = old('metode_pembayaran', $pendaftar->metode_pembayaran ?? ''); @endphp
                        <div class="field" id="metodePembayaranFieldWrap" style="{{ $gelombang == 'Gelombang 2' ? '' : 'display:none;' }}">
                            <label>Metode Pembayaran (Biaya Gel. 2)<span class="req">*</span></label>
                            <select name="metode_pembayaran" class="req" id="select_metodePembayaran">
                                <option value="" disabled {{ !$metodePembayaran ? 'selected' : '' }}>Pilih Metode Pembayaran</option>
                                <option value="langsung" {{ $metodePembayaran == 'langsung' ? 'selected' : '' }}>Bayar Langsung di Sekolah (Cash)</option>
                                <option value="transfer" {{ $metodePembayaran == 'transfer' ? 'selected' : '' }}>Transfer Bank (Via TF)</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="section">
                    ...
                    <div class="row-2">
                        <div class="field"><label>Asal Sekolah<span class="req">*</span></label><input type="text" name="asal_sekolah" class="req" value="{{ old('asal_sekolah', $pendaftar->asal_sekolah) }}"></div>
                        <div class="field"><label>Nilai Rata-rata Raport<span class="req">*</span></label><input type="number" name="nilai_rata_raport" class="req" step="0.01" min="0" max="100" placeholder="cth. 85.50" value="{{ old('nilai_rata_raport', $pendaftar->nilai_rata_raport ?? '') }}"></div>
                    </div>
                </div>
    ...
    <script>
        ...
        // Tampilkan/sembunyikan field Metode Pembayaran sesuai Gelombang yang dipilih.
        function toggleMetodePembayaranField(gelombangVal) {
            const wrap = document.getElementById('metodePembayaranFieldWrap');
            if (!wrap) return;
            wrap.style.display = (gelombangVal === 'Gelombang 2') ? '' : 'none';
        }

        // ---------- DROPDOWN WILAYAH (PROVINSI/KABUPATEN/KECAMATAN/KELURAHAN) ----------
        // Sumber data: API Wilayah Indonesia (emsifa), sama seperti formulir.html &
        // detail_pendaftar_admin.html. Nilai yang tersimpan di database bisa berupa ID
        // wilayah emsifa maupun nama teks (data lama), jadi pencocokannya mendukung keduanya.
        const baseWilayahAPI = 'https://www.emsifa.com/api-wilayah-indonesia/api';

        const selProvinsi = document.getElementById('select_provinsi');
        const selKabupaten = document.getElementById('select_kabupaten');
        const selKecamatan = document.getElementById('select_kecamatan');
        const selKelurahan = document.getElementById('select_kelurahan');

        const provinsiTersimpan = @json($pendaftar->provinsi ?? '');
        const kabupatenTersimpan = @json($pendaftar->kabupaten ?? '');
        const kecamatanTersimpan = @json($pendaftar->kecamatan ?? '');
        const desaTersimpan = @json($pendaftar->desa ?? '');

        function findWilayahMatch(list, idOrName) {
            if (!idOrName) return null;
            const str = String(idOrName);
            let match = list.find(w => String(w.id) === str);
            if (!match) {
                match = list.find(w => w.name.toLowerCase() === str.toLowerCase());
            }
            return match || null;
        }

        // Kalau nilai lama tidak ketemu di daftar resmi, tetap tampilkan sebagai opsi
        // custom (supaya data lama tidak hilang begitu saja saat form dibuka).
        function addCustomOptionIfNeeded(selectEl, rawValue, matchedId) {
            if (!rawValue || matchedId) return;
            const opt = document.createElement('option');
            opt.value = rawValue;
            opt.textContent = rawValue + " (belum sesuai data wilayah resmi)";
            opt.selected = true;
        ...
        function loadKabupatenEdit(provId, selectedKabIdOrName = null) {
            selKabupaten.innerHTML = '<option value="">Pilih Kabupaten</option>';
            selKecamatan.innerHTML = '<option value="">Pilih Kecamatan</option>';
            selKelurahan.innerHTML = '<option value="">Pilih Kelurahan/Desa</option>';
            selKecamatan.disabled = true;
            selKelurahan.disabled = true;

            if (!provId) { selKabupaten.disabled = true; return Promise.resolve(); }

            return fetch(baseWilayahAPI + '/regencies/' + provId + '.json')
                .then(res => res.json())
                .then(regencies => {
                    populateSelect(selKabupaten, regencies, "Pilih Kabupaten");
                    selKabupaten.disabled = false;
                    if (selectedKabIdOrName) {
                        const match = findWilayahMatch(regencies, selectedKabIdOrName);
                        if (match) selKabupaten.value = match.id;
                        else addCustomOptionIfNeeded(selKabupaten, selectedKabIdOrName, null);
                        return match ? match.id : null;
                    }
                    return null;
                })
                .catch(err => { console.error('Gagal memuat kabupaten:', err); return null; });
        }

        function loadKecamatanEdit(kabId, selectedKecIdOrName = null) {
            selKecamatan.innerHTML = '<option value="">Pilih Kecamatan</option>';
            selKelurahan.innerHTML = '<option value="">Pilih Kelurahan/Desa</option>';
            selKelurahan.disabled = true;

            if (!kabId) { selKecamatan.disabled = true; return Promise.resolve(); }

            return fetch(baseWilayahAPI + '/districts/' + kabId + '.json')
                .then(res => res.json())
                .then(districts => {
                    populateSelect(selKecamatan, districts, "Pilih Kecamatan");
                    selKecamatan.disabled = false;
                    if (selectedKecIdOrName) {
                        const match = findWilayahMatch(districts, selectedKecIdOrName);
                        if (match) selKecamatan.value = match.id;
                        else addCustomOptionIfNeeded(selKecamatan, selectedKecIdOrName, null);
                        return match ? match.id : null;
                    }
                    return null;
                })
                .catch(err => { console.error('Gagal memuat kecamatan:', err); return null; });
        }

        function loadKelurahanEdit(kecId, selectedKelIdOrName = null) {
            selKelurahan.innerHTML = '<option value="">Pilih Kelurahan/Desa</option>';

            if (!kecId) { selKelurahan.disabled = true; return Promise.resolve(); }

            return fetch(baseWilayahAPI + '/villages/' + kecId + '.json')
                .then(res => res.json())
                .then(villages => {
                    populateSelect(selKelurahan, villages, "Pilih Kelurahan/Desa");
                    selKelurahan.disabled = false;
                    if (selectedKelIdOrName) {
                        const match = findWilayahMatch(villages, selectedKelIdOrName);
                        if (match) selKelurahan.value = match.id;
                        else addCustomOptionIfNeeded(selKelurahan, selectedKelIdOrName, null);
                    }
                })
                .catch(err => console.error('Gagal memuat kelurahan:', err));
        }

        selProvinsi.addEventListener('change', function() { loadKabupatenEdit(this.value); });
        selKabupaten.addEventListener('change', function() { loadKecamatanEdit(this.value); });
        selKecamatan.addEventListener('change', function() { loadKelurahanEdit(this.value); });

        let provinsiListCache = [];
        const provinsiReady = fetch(baseWilayahAPI + '/provinces.json')
            .then(res => res.json())
            .then(provinces => {
                provinsiListCache = provinces;
                populateSelect(selProvinsi, provinces, "Pilih Provinsi");
            })
            .catch(err => console.error('Gagal memuat provinsi:', err));

        // Preselect otomatis berdasarkan data yang sudah tersimpan di server (Blade),
        // bukan localStorage seperti versi statis.
        provinsiReady.then(async () => {
            if (!provinsiTersimpan) return;

            const matchProv = findWilayahMatch(provinsiListCache, provinsiTersimpan);
            if (matchProv) {
                selProvinsi.value = matchProv.id;
            } else {
                addCustomOptionIfNeeded(selProvinsi, provinsiTersimpan, null);
                return;
            }

            const kabId = await loadKabupatenEdit(matchProv.id, kabupatenTersimpan);
            if (!kabId) return;

            const kecId = await loadKecamatanEdit(kabId, kecamatanTersimpan);
            if (!kecId) return;

            await loadKelurahanEdit(kecId, desaTersimpan);
        });
        ...
        function validateForm() {
            ...
            document.querySelectorAll("input.req, select.req").forEach(el => {
                if (el.offsetParent === null) return; // lewati field yang sedang disembunyikan (mis. Metode Pembayaran saat Gelombang 1)
